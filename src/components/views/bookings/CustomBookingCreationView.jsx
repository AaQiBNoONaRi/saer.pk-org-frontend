...
const CustomBookingCreationView = ({ onBookCustomPackage }) => {
                ...
                <div className="animate-in fade-in zoom-in duration-500 space-y-10">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                onClick={() => onBookCustomPackage && onBookCustomPackage({ families, hotelRows, selectedFlight, selectedVehicle, selectedVisaRate, riyalRate, selectedOptions, includeFood, includeZiarat, foodRows, ziaratRows, foodPrices, ziaratPrices, passengers })}
                                ...
                                <Send size={15} /> Book Custom Umrah
                            </button>
                        </div>
                    </div>
                            ...
                            <div key={family.id} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
                                ...
                                <div className="p-8 space-y-8">
                                    ...
                                    {foodRows.some(r => r.food_id) && (
                                        <div>
                                            <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-3 pb-2 border-b border-slate-100">Food Services</h4>
                                            <div className="overflow-x-auto">
                                                <table className="w-full text-left text-xs">
                                                    <thead>
                                                        <tr className="border-b border-slate-100">
                                                            {['City', 'Meal Package', 'Start Date', 'End Date', 'Days', `Rate/Adult (${inv.isFoodPKR ? 'PKR' : 'SAR'})`, 'Total (PKR)'].map(h => (
                                                                <th key={h} className="py-2 pr-4 font-black text-slate-400 uppercase tracking-wider whitespace-nowrap">{h}</th>
                                                            ))}
                                                        </tr>
                                                    </thead>
                                                    <tbody className="font-bold text-slate-700">
                                                        {foodRows.filter(r => r.food_id).map((foodRow, fri) => {
                                                            const item = foodPrices.find(f => String(f.id || f._id) === String(foodRow.food_id));
                                                            ...
                                                            const adultRate = item?.adult_selling || 0;
                                                            const isFoodPKR = inv.isFoodPKR;
                                                            const toPKR = (v) => isFoodPKR ? v : Math.round(v * inv.exchangeRate);
                                                            const rowTotal = toPKR(
                                                                (adultRate * (family.adults || 0) +
                                                                    (item?.child_selling || 0) * (family.children || 0) +
                                                                    (item?.infant_selling || 0) * (family.infants || 0)) * days
                                                            );
                                                            return (
                                                                <tr key={fri} className="border-b border-slate-50">
                                                                    <td className="py-2 pr-4">{foodRow.city || '—'}</td>
                                                                    <td className="py-2 pr-4">{item?.name || item?.title || foodRow.food_id}</td>
                                                                    <td className="py-2 pr-4">{foodRow.startDate || '—'}</td>
                                                                    <td className="py-2 pr-4">{foodRow.endDate || '—'}</td>
                                                                    <td className="py-2 pr-4">{days}</td>
                                                                    <td className="py-2 pr-4">{isFoodPKR ? 'PKR' : 'SAR'} {adultRate?.toLocaleString()}</td>
                                                                    <td className="py-2 pr-4">PKR {rowTotal?.toLocaleString()}</td>
                                                                </tr>
                                                            );
                                                        })}
                                                        <tr className="border-t border-slate-200 bg-slate-50">
                                                            <td className="py-2 pr-4 font-black text-slate-900" colSpan={6}>Total Food</td>
                                                            <td className="py-2 pr-4 font-black text-slate-900">PKR {inv.foodPKR?.toLocaleString()}</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                    ...
                                    {ziaratRows.some(r => r.ziarat_id) && (
                                        <div>
                                            <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-3 pb-2 border-b border-slate-100">Ziyarat Tours</h4>
                                            <div className="overflow-x-auto">
                                                <table className="w-full text-left text-xs">
                                                    <thead>
                                                        <tr className="border-b border-slate-100">
                                                            {['City', 'Tour', 'Visit Date', `Rate/Adult (${inv.isZiaratPKR ? 'PKR' : 'SAR'})`, 'Total (PKR)'].map(h => (
                                                                <th key={h} className="py-2 pr-4 font-black text-slate-400 uppercase tracking-wider whitespace-nowrap">{h}</th>
                                                            ))}
                                                        </tr>
                                                    </thead>
                                                    <tbody className="font-bold text-slate-700">
                                                        {ziaratRows.filter(r => r.ziarat_id).map((zRow, zi) => {
                                                            const item = ziaratPrices.find(z => String(z.id || z._id) === String(zRow.ziarat_id));
                                                            const isZiaratPKR = inv.isZiaratPKR;
                                                            const toPKR = (v) => isZiaratPKR ? v : Math.round(v * inv.exchangeRate);
                                                            const adultRate = item?.adult_selling || 0;
                                                            const rowTotal = toPKR(
                                                                adultRate * (family.adults || 0) +
                                                                (item?.child_selling || 0) * (family.children || 0) +
                                                            ...
                                                            return (
                                                                <tr key={zi} className="border-b border-slate-50">
                                                                    <td className="py-2 pr-4">{zRow.city || '—'}</td>
                                                                    <td className="py-2 pr-4">{item?.name || item?.title || zRow.ziarat_id}</td>
                                                                    <td className="py-2 pr-4">{zRow.visitDate || '—'}</td>
                                                                    <td className="py-2 pr-4">{isZiaratPKR ? 'PKR' : 'SAR'} {adultRate?.toLocaleString()}</td>
                                                                    <td className="py-2 pr-4">PKR {rowTotal?.toLocaleString()}</td>
                                                                </tr>
                                                            );
                                                        })}
                                                        <tr className="border-t border-slate-200 bg-slate-50">
                                                            <td className="py-2 pr-4 font-black text-slate-900" colSpan={4}>Total Ziyarat</td>
                                                            <td className="py-2 pr-4 font-black text-slate-900">PKR {inv.ziaratPKR?.toLocaleString()}</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
...
export default CustomBookingCreationView;
