...
import { ArrowLeft, Save, CreditCard, Calendar, Hotel, X, Plus } from 'lucide-react';
...
const AddServiceChargeView = ({ onBack, initialData }) => {
    ...
    // Service Charge Periods (Repeatable)
    const [chargePeriods, setChargePeriods] = useState([
        {
            id: Date.now(),
            ticketCharge: 0,
            packageCharge: 0,
    ...
    useEffect(() => {
        if (initialData) {
            ...
            if (initialData.charge_periods && initialData.charge_periods.length > 0) {
                setChargePeriods(initialData.charge_periods);
            }
        }
    }, [initialData]);
    ...
    const addChargePeriod = () => {
        const lastPeriod = chargePeriods[chargePeriods.length - 1];
        const newPeriod = {
            id: Date.now(),
            ticketCharge: 0,
            packageCharge: 0,
        ...
        setChargePeriods([...chargePeriods, newPeriod]);
    };

    const removeChargePeriod = (index) => {
        setChargePeriods(chargePeriods.filter((_, i) => i !== index));
    };
        ...
        <div className="max-w-6xl mx-auto p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tight">
                        {initialData ? 'Edit Service Charge' : 'Add Service Charge'}
                    </h2>
                    <p className="text-slate-500 font-medium">Configure service charge rates and hotel pricing</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Applies to all service charge periods</p>
                        </div>
                    </div>
                    ...
                    <div key={period.id} className="space-y-6 relative">
                        ...
                        <div className="bg-gradient-to-r from-blue-50 to-purple-50 px-6 py-3 rounded-2xl">
                            <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight text-center">
                                Service Charge Period #{periodIndex + 1}
                            </h2>
                        </div>

                        {/* SECTION 1: Service Charge Amounts */}
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-slate-900 tracking-tight">Service Charge Amounts</h3>
                                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Fixed Charge Values (PKR)</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">
                                        Ticket Service Charge (PKR)
                                    </label>
                                    <input
                                        type="number"
                                        value={period.ticketCharge}
                                        onChange={(e) => updatePeriodField(periodIndex, 'ticketCharge', Number(e.target.value) || 0)}
                                        placeholder="0"
                                        min="0"
                                ...
                                <div>
                                    <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">
                                        Package Service Charge (PKR)
                                    </label>
                                    <input
                                        type="number"
                                        value={period.packageCharge}
                                        onChange={(e) => updatePeriodField(periodIndex, 'packageCharge', Number(e.target.value) || 0)}
                                        placeholder="0"
                                        min="0"
                                        className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none font-bold text-sm"
                                    />
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Service Charge Price Valid Dates</p>
                                </div>
                            </div>
                ...
                <div className="flex gap-4">
                    ...
                    >
                        <Plus size={18} /> Add Service Charge Period
                    </button>
                    <button
                        type="submit"
                        disabled={saving}
